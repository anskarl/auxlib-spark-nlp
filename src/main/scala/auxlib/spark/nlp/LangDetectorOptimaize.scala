...
trait LangDetector extends Transformer with Params {

  final val inCol =
    new Param[String](this, "text", "The name of the input column")

  final val outCol =
    new Param[String](this, "language", "The name of the output language column, the output is encoded in ISO 693-3")
...
class LangDetectorOptimaize(
  override val uid: String,
  langDetectBuilder: () => LanguageDetector
) extends LangDetector {

  def this() = this(Identifiable.randomUID("language-detector"), LangDetector.Optimaize.defaultLangBuilder)
      ...
      val languageDetector = langDetectBuilder()
        ...
        iter.map{ row =>
          ...
          val result: Option[String] = Option(row.getAs[String](${inCol})).flatMap{ text =>
            ...
            languageDetector.detect(textObject).asScala.map(l => new Locale(l.getLanguage).getISO3Language)
          }
...
class LangDetectorOpenNLP(override val uid: String) extends LangDetector {

  def this() = this(Identifiable.randomUID("language-detector"))

  final val modelResource =
    new Param[String](this, "model_resource", "The resource of the Language Detection model")

  def setModelResource(value: String): this.type = set(modelResource, value)

  setDefault(modelResource, "/langdetect-183.bin")
  ...
  override def transform(dataset: Dataset[_]): DataFrame = {
    ...
    val resultingRdd = inputRDD.mapPartitions { iter =>
      ...
      val detector = {
        val is = this.getClass.getResourceAsStream(${modelResource})
        val model = try new LanguageDetectorModel(is) finally is.close()
        new LanguageDetectorME(model)
      }

      iter.map{ row =>
        val txt = row.getAs[String](${inCol})
        val bestPredictedLanguage = detector.predictLanguage(txt)

        val result = Option(bestPredictedLanguage.getLang)
...
object LangDetector {
  ...
  object OpenNLP{
    def apply(): LangDetectorOpenNLP = new LangDetectorOpenNLP()
    def create(): LangDetectorOpenNLP = new LangDetectorOpenNLP()
  }

  object Optimaize {
    def apply(): LangDetectorOptimaize = new LangDetectorOptimaize()

    def create(): LangDetectorOptimaize = new LangDetectorOptimaize()

    def create(builder: LanguageDetectorBuilder => LanguageDetector): LangDetectorOptimaize = {
      new LangDetectorOptimaize(() => builder(LanguageDetectorBuilder.create(NgramExtractors.standard())))
    }

    protected[nlp] def defaultLangBuilder: () => LanguageDetector = {
